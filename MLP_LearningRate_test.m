...
path2 = '...'; % DeepLearnToolbox path
cd(path2);

rand('state',0)

lr_options = [0.1 0.5 1]; % Define learning rate options
count = length(lr_options);
...
for i = 1 : count
    ...
    nn.output = 'softmax';              % Choose : 'sigm' or 'softmax'
    nn.momentum = 0.5;
    nn.dropoutfraction = 0;
    nn.learningRate = lr_options(i);              %  Sigm require a lower learning rate
...
figure;
plot(lr_options, my_train_error, lr_options, my_test_error);
